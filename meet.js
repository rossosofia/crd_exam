...
function triggerButtons() {
  document
    ...
    .forEach((each) => each.addEventListener("click", selectFilter));
}
...
function selectFilter(event) {
  ...
  setFilter(filter);
  updateActiveFilterButton(event.target); // Pass the clicked button
}
...
function filterList(filteredList) {
  if (globalObject.filterBy !== "*") {
    filteredList = allMembers.filter(function whichMember(member) {
      // It's a different way with closure
